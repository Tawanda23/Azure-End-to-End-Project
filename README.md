...
- **Monitoring:** Azure Monitor is used as a comprehensive monitoring solution for the AKS cluster.
...
For the container ochestration part of the project, Kubernetes, a tool which automates the task of managing containerised applications will be used. Kubernetes has the following features:

- Multi-container deployments across a cluster which allows users to specify the number containers that should be deployed. Users are also able to specifying container replicas for fault tolerance.

- Self-healing: is a feature of Kubernetes that enables a pod to spin up a replacement if and when it goes down.

- Autoscaling: can use Horizontal Pod Autoscaler to scale the number of pod replicas to meet changing demands. Horizontal scalling means it will create a new pod whilst vertical scaling the pod will have an increase or decrease in CPU and RAM depending on need.

- Load Balancing: Kubernetes is able to distribute traffic amongst several worker nodes to ensure high availability and efficient resource utilisation.

The diagram below shows a Kubernetes Architecture:
